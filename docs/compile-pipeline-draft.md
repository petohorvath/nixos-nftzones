...
2. **`sroute` emits an invalid chain type — blocked on nftypes upstream.** `internal.emit.chainTypeOf` (`lib/internal/emit.nix:356-369`) classifies any `(prerouting|output, mangle)` chain as `type route`. `sroute` dispatches to `prerouting + mangle` (`lib/internal/dispatch.nix:155-158`), so the emitted ruleset declares a `route` chain at `prerouting` — which the kernel rejects ("Chain of type 'route' is not supported"). `route` chains are valid only at the `output` hook (per `man nft` Table 6 and `docs/nftables-structure.md:81`). The §3.1 dispatch table at line 164 and the §4.2 placement note at line 197 both inherit the same misconception.

   The fix can't land here cleanly because the missing data point — "which hooks does each chain type permit?" — isn't surfaced by `nftypes.compatibility`. The right shape is a `hooksByChainType` constant alongside the existing `familiesByChainType` and `hooksByFamily`. Tracking the upstream proposal at `../nix-nftypes/prompt-fix.md`. When that lands, `chainTypeOf` should consult it (and probably move the derivation upstream entirely as a `chainTypeFor :: hook -> priority -> chainType` helper), and `sroute` should fall through to `type filter` at `prerouting + mangle` (mark-set-then-routing-table-lookup-via-`ip rule` is the kernel-correct shape; `route` re-evaluation is meaningless for inbound packets that already chose their interface).

   Coverage: the `tests/integration/scenarios/sroute-*.nix` slot is intentionally empty until the fix lands. `droute` (output + mangle) renders correctly and is covered.

3. **`bridge` family rulesets would silently misbehave — blocked on nftypes upstream.** Bridge has its own priority constants (`nftypes.compatibility.priorityIntsBridge`: `filter = -200`, `srcnat = 300`, `dstnat = -300`, `out = 100`) but `internal.dispatch.canonicalPriority` and `internal.emit.chainTypeOf` only consult `priorityIntsDefault`. A bridge table at `(hook = postrouting, priority = 300)` would canonicalize to `"postrouting-at-300"` (instead of `"postrouting-at-srcnat"`) and derive `chainType = "filter"` (instead of `"nat"`). The emitted `prio` field IS family-aware because it goes through `nftypes.resolvePriority family priority`, so the kernel sees the right int — but the chain *name* and *type* end up wrong, and the user has no way to tell from the symptom which constant is mismapped.

   Phase 1's `checkSupportedFamily` rejects `family = "bridge"` at compile time today so users get a clear error instead of silent misbehaviour. The proper fix needs upstream nftypes to expose family-aware reverse lookups (an inverse of `priorityIntsByFamily` for canonicalization, plus a `chainTypeFor :: family -> hook -> priority -> chainType-or-null` derivation). Tracking the upstream proposal at `../nix-nftypes/prompt-fix-2.md`. When that lands, drop `"bridge"` from `checkSupportedFamily.unsupported`, replace the inline tables in `dispatch.nix` and `emit.nix` with the upstream helpers, and add bridge integration scenarios.
