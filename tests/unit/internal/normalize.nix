...
in
{
  ...
  # ===== checkMatchOverrideContent — no override sections produces no errors =====

  testCheckMatchOverrideContentNoOverrides = {
    expr =
      (runEvalPipeline
        [
          convertNodesToZones
  ...
  # ===== checkMatchOverrideContent — pure match clauses accepted =====

  testCheckMatchOverrideContentMatchAccepted = {
    # All four sections populated with match clauses only — every
    # statement is the `{ match = ...; }` shape produced by `dsl.eq`
    # / `dsl.inSet`. Nothing should fire.
    expr =
      (runEvalPipeline
        [
          convertNodesToZones
        ...
        ]
        {
          zones.lan = {
            ...
            matchOverride.ingress = {
              interfaces = [ (dsl.eq dsl.fields.meta.iifname "lan0") ];
              ipv4 = [ (dsl.eq dsl.fields.ip.saddr "10.0.0.0/24") ];
              ipv6 = [ (dsl.eq dsl.fields.ip6.saddr "fe80::/64") ];
              extra = [ (dsl.eq dsl.fields.meta.mark 256) ];
            };
  ...
  # ===== checkMatchOverrideContent — accept verdict in extra rejected =====

  testCheckMatchOverrideContentAcceptInExtra = {
    expr =
      let
        errors =
          (runEvalPipeline
      ...
      in
      {
        count = builtins.length errors;
        name = (builtins.head errors).name;
        path = pkgs.lib.hasInfix "zones.lan.matchOverride.ingress.extra[1]" (builtins.head errors).value;
        kind = pkgs.lib.hasInfix "is a `accept` statement" (builtins.head errors).value;
      };
    expected = {
      count = 1;
      name = "matchOverrideNonMatch";
      path = true;
      kind = true;
    };
  };

  # ===== checkMatchOverrideContent — jump verdict in extra rejected =====

  testCheckMatchOverrideContentJumpInExtra = {
    expr =
      let
        errors =
          (runEvalPipeline
            ...
            ]
            {
              zones.lan = {
                interfaces = [ "lan0" ];
                matchOverride.ingress.extra = [ (dsl.jump "bypass") ];
              };
            }
          ).errors;
      in
      {
        count = builtins.length errors;
        name = (builtins.head errors).name;
        kind = pkgs.lib.hasInfix "is a `jump` statement" (builtins.head errors).value;
      };
    expected = {
      count = 1;
      name = "matchOverrideNonMatch";
  ...
  # ===== checkMatchOverrideContent — non-match flagged in every section =====

  testCheckMatchOverrideContentAllSections = {
    # One non-match statement per section — verify the validator
    # walks all four (interfaces / ipv4 / ipv6 / extra) on both
    # sides, not just one.
    expr =
      let
        errors =
          (runEvalPipeline
            ...
            ]
            {
              zones.lan = {
                interfaces = [ "lan0" ];
                matchOverride.ingress = {
                  interfaces = [ dsl.accept ];
                  ipv4 = [ dsl.drop ];
                  ipv6 = [ (dsl.jump "x") ];
                  extra = [ (dsl.goto "y") ];
                };
                matchOverride.egress.extra = [ dsl.accept ];
              };
            }
          ).errors;
        paths = map (e: e.value) errors;
      in
      {
        count = builtins.length errors;
        ingressInterfaces = pkgs.lib.any (pkgs.lib.hasInfix "ingress.interfaces[0]") paths;
        ingressIpv4 = pkgs.lib.any (pkgs.lib.hasInfix "ingress.ipv4[0]") paths;
        ingressIpv6 = pkgs.lib.any (pkgs.lib.hasInfix "ingress.ipv6[0]") paths;
        ingressExtra = pkgs.lib.any (pkgs.lib.hasInfix "ingress.extra[0]") paths;
        egressExtra = pkgs.lib.any (pkgs.lib.hasInfix "egress.extra[0]") paths;
        allTagged = pkgs.lib.all (e: e.name == "matchOverrideNonMatch") errors;
      };
    expected = {
      count = 5;
      ingressInterfaces = true;
      ingressIpv4 = true;
      ingressIpv6 = true;
      ingressExtra = true;
      egressExtra = true;
  ...
  # ===== checkMatchOverrideContent — errors aggregate across zones =====

  testCheckMatchOverrideContentMultipleZones = {
    expr =
      let
        errors =
          (runEvalPipeline
            ...
            ]
            {
              zones = {
                lan = {
                  interfaces = [ "lan0" ];
                  matchOverride.ingress.extra = [ dsl.accept ];
                };
                wan = {
                  interfaces = [ "wan0" ];
                  matchOverride.egress.extra = [ dsl.drop ];
                };
              };
            }
          ).errors;
      ...
      {
        count = builtins.length errors;
        lan = pkgs.lib.any (pkgs.lib.hasInfix "zones.lan.matchOverride.ingress.extra") paths;
        wan = pkgs.lib.any (pkgs.lib.hasInfix "zones.wan.matchOverride.egress.extra") paths;
      };
    expected = {
      count = 2;
      lan = true;
